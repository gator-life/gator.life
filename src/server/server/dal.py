...
class Dal(object):
    ...
    ds_client = _datastore_client()

    def _make_entity(self, entity_type, not_indexed):
        key = self.ds_client.key(entity_type)
    ...
    def _make_named_entity(self, entity_type, key_name, not_indexed):
        key = self.ds_client.key(entity_type, key_name)
        return datastore.entity.Entity(key, exclude_from_indexes=not_indexed)
    ...
    def get_user_and_password(self, email):
        ...
        """
        key = self.ds_client.key('User', email)
        db_user = self.ds_client.get(key)
    ...
    def get_all_users(self):
        ...
        """
        all_users_query = self.ds_client.query(kind='User').fetch()
    ...
    def save_user(self, user, password):
        ...
        if user._user_doc_set_db_key is None:  # pylint: disable=protected-access
            ...
            self.ds_client.put(user_doc_set_db)
        ...
        if user._user_computed_profile_db_key is None:  # pylint: disable=protected-access
            ...
            self.ds_client.put(db_user_computed_profile)
    ...
    def get_features(self, feature_set_id):
        ...
        """
        db_feature_set_key = self.ds_client.key(u'FeatureSet', feature_set_id)
        db_feature_set = self.ds_client.get(db_feature_set_key)
    ...
    def save_features(self, feature_set_id, feature_names):
        ...
        self.ds_client.put(db_feature_set)
    ...
    def _to_db_feature_vector(self, feature_vector):
        ...
        db_feature_vector['feature_set_key'] = self.ds_client.key(u'FeatureSet', feature_vector.feature_set_id)
    ...
    def save_computed_user_profiles(self, user_profile_list):
        ...
        self.ds_client.put_multi(db_profiles)
    ...
    def get_user_computed_profiles(self, users):
        ...
        db_profiles = self.ds_client.get_multi(keys)
    ...
    def _to_docs(self, db_doc_keys):
        db_docs = self.ds_client.get_multi(db_doc_keys)
    ...
    def save_user_docs(self, user, user_docs):
        ...
        db_user_doc_set = self.ds_client.get(user_doc_set_db_key)
        db_user_docs = self._to_db_user_docs(user_docs)
        db_user_doc_set['user_documents'] = db_user_docs
        self.ds_client.put(db_user_doc_set)
    ...
    def save_users_docs(self, user_to_user_docs_list):
        ...
        db_user_sets = self.ds_client.get_multi(user_set_db_keys)
        ...
        self.ds_client.put_multi(db_user_sets)

    def get_user_docs(self, user):
        user_doc_set = self.ds_client.get(user._user_doc_set_db_key)  # pylint: disable=protected-access
    ...
    def get_users_docs(self, users):
        ...
        db_user_doc_sets = self.ds_client.get_multi(user_doc_set_db_keys)
    ...
    def save_documents(self, documents):
        ...
        self.ds_client.put_multi(db_docs)
    ...
    def get_doc_by_urlsafe_key(self, key):
        ...
        db_doc = self.ds_client.get(db_key)
    ...
    def get_user_actions_on_docs(self, users, from_datetime):
        ...
        actions_query = self.ds_client.query(kind='UserActionOnDoc')
