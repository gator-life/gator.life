...
from topicmodeller.topicmodeller import TopicModeller, _decode, _filter_latin_words, _readable_document, _remove_stop_words, \
    _word_tokenize
...
class TopicModellerTests(unittest.TestCase):
    tm_documents = [ ([], [ [u'BattleBots', u'producers', u'definitely', u'looking', u'forward', u'feeling',
               u'confident', u'ABC', u'renew', u'series', u'second', u'season', u'seventh', u'count', u'five',
               u'seasons', u'ran', u'Comedy', u'Central', u'And', u'\'ve', u'definitely',
               u'given', u'thought', u'like', u'similarly', u'like', u'change'],
    ...
    def setUp(self):
        self.scraper_document = _decode(open('scraper_documents/2015-08-01 18:00:22.926317_8.json').read())

        for file in os.listdir('tm_data'):
            os.remove(os.path.join('tm_data', file))

    def test_readable_document(self):
        readable_document = _readable_document(self.scraper_document.html_content)
    ...
    def test_word_tokenize(self):
        document = u"\"BattleBots\" producers are definitely looking forward and feeling confident that ABC will renew the " \
                   u"series for a second season (seventh if you count the five seasons it ran on Comedy Central from 2000 to " \
                   u"2002). And they've definitely given some thought as to what they'd like to do similarly and what they'd " \
                   u"like to change up."
        word_tokenized = _word_tokenize(document)

        words = [u'will', u'renew', u'the', u'series', u'for', u'a', u'second', u'season', u'seventh', u'if', u'you', u'count']
    ...
    def test_filter_latin_words(self):
        words = [u'I', u'don\'t', u'know', u'how', u'to', u'do', u'it']
    ...
    def test_initialize_tm_dictionary(self):
        tm = TopicModeller('tm_data', 2)
        tm.initialize_dictionary(self.tm_documents)

        all_words = (word for (_, batched_document) in self.tm_documents
                   for document in batched_document
                   for word in document)

        self.assertTrue(all(word in tm.dictionary.values() for word in all_words))
